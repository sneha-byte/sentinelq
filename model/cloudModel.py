...
def main():
    ...
    for result in model.track(source=0, stream=True, verbose=False, show=True):        
        if result.boxes is None:
            continue
            
        raw_frame = result.orig_img
        is_dark = is_low_light(raw_frame)
        
        # If low-light, enhance the frame and re-run inference on it
        # This gives YOLO a much better image to work with
        if is_dark:
            enhanced = enhance_frame(raw_frame)
            results2 = model.predict(enhanced, verbose=False)
            result   = results2[0]
