...
export function IncidentsView({
  ...
  // ── Shared list panel (reused on both desktop and mobile) ─────────────────
  function ListPanel() {
    return (
...
export function IncidentsView({ acknowledged, alertSent, onAcknowledge, onAlertSent }: IncidentsViewProps) {
  ...
  const SORT_OPTIONS = [
    { id: "combined" as SortMode, label: "Smart",  Icon: Sparkles },
    { id: "danger"   as SortMode, label: "Danger", Icon: ShieldAlert },
    { id: "time"     as SortMode, label: "Recent", Icon: Clock },
  ];

  function enriched(inc: Incident) {
    return { ...inc, acknowledged: inc.acknowledged || acknowledged.has(inc.id) };
  }

  return (
    <>
      {/* ── Desktop: side by side, full height ── */}
      <div className="hidden md:grid md:grid-cols-[360px_1fr] gap-5" style={{ height: "calc(100vh - 112px)" }}>
        ...
        <div className="flex flex-col min-h-0 rounded-xl border border-border bg-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-border px-4 py-3 shrink-0">
            ...
            <span className="rounded-full bg-secondary px-2 py-0.5 text-xs font-medium text-muted-foreground">{sorted.length}</span>
          </div>
          <div className="flex items-center gap-1 border-b border-border px-3 py-2 shrink-0">
            {(["all", "active", "resolved"] as Filter[]).map(f => (
              <button key={f} onClick={() => setFilter(f)}
      ...
      <div className="flex flex-col gap-3 md:hidden">
        {selectedIncident ? (
          ...
          <div className="flex flex-col rounded-xl border border-border bg-card overflow-hidden">
            ...
            <div className="flex flex-col gap-2 p-3">
              {sorted.map(inc => (
                <IncidentCard key={inc.id} incident={enriched(inc)} onSelect={setSelectedIncident} selected={false} />
              ))}
            </div>
          </div>
        )}
