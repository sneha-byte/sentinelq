...
export function CamerasView({
  lightStates = {}, autoMotion = {},
  onToggleLight = () => {}, onToggleAutoMotion = () => {},
}: CamerasViewProps) {
  ...
  // --- FIX: Added local state to manage the light and auto-motion toggles ---
  ...
  const handleToggleLight = (id: string) => {
    ...
    onToggleLight(id); // Still call the parent prop if provided
  };

  const handleToggleAutoMotion = (id: string) => {
    setLocalAutoMotion(prev => ({ ...prev, [id]: !prev[id] }));
    onToggleAutoMotion(id); // Still call the parent prop if provided
  };
  // --------------------------------------------------------------------------
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center rounded-lg border border-border bg-card p-0.5">
              ...
              <button onClick={() => setLayout("large")}
                className={cn("rounded-md p-1.5 transition-colors",
        ...
        <div className={cn("grid gap-4",
          ...
          {filteredCameras.map(cam => (
            <div key={cam.id} className="flex flex-col rounded-xl overflow-hidden border border-border">
              <div onClick={() => { setSelectedCamId(cam.id); setSelectedIncident(null); }}
  ...
  return (
    <div className="flex flex-col gap-5">
      <button onClick={() => { setSelectedCamId(null); setSelectedIncident(null); }}
      ...
      <div className="rounded-xl border border-primary/15 bg-card overflow-hidden max-w-xl">
        <CameraFeedCard camera={selectedCam!} />
        ...
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1.5 border-t border-border bg-secondary/30 px-4 py-2.5">
          ...
          <span className={cn("rounded-md px-1.5 py-0.5 text-[11px] font-medium",
            ...
            {selectedCam!.routeMode === "LOCAL" ? "Edge" :
             selectedCam!.routeMode === "CLOUD" ? "Cloud" : "Hybrid"}
          </span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2 pt-1">
        ...
        <h3 className="text-sm font-semibold text-foreground">Incidents — {selectedCam!.name}</h3>
...
function LightControls({
  cameraId, isOn, isAuto, onToggleLight, onToggleAuto, compact = false,
}: {
  cameraId:       string;
  isOn:           boolean;
  isAuto:         boolean;
  ...
  return (
    <div className={cn(
      "flex gap-2",
      compact ? "px-3 py-2 bg-secondary/30 border-t border-border" : ""
    )}>
      {/* Manual on/off */}
      <button
        onClick={e => { e.stopPropagation(); onToggleLight(cameraId); }}
        className={cn(
      ...
      {/* Auto motion */}
      <button
        onClick={e => { e.stopPropagation(); onToggleAuto(cameraId); }}
        className={cn(
          "flex flex-1 items-center justify-center gap-1.5 rounded-lg border px-3 py-2 text-xs font-medium transition-all",
          isAuto
